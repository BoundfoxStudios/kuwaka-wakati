...
export default class ImporterComponent {
    ...
    async import(): Promise<void> {
        ...
        for (const line of lines) {
            ...
            await this.timeTable.add({ utcDate, start: start1Millis, end: end1Millis, isNonWorkday: false, isADayOff: false });

            if (start2 !== end2) {
                const end2Millis = DateTime.fromFormat(end2, 'HH:mm').toMillis();
                const start2Millis = DateTime.fromFormat(start2, 'HH:mm').toMillis();
                await this.timeTable.add({ utcDate, start: start2Millis, end: end2Millis, isNonWorkday: false, isADayOff: false });
            }
